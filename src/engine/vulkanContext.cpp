...
void VulkanContext::Init(const char* title)
{
	...
	appInfo.apiVersion =
		VK_API_VERSION_1_0; // the highest version the application is designed to use
	...
	if (Config::enableValidationLayers)
	{
		...
		instanceInfo.pNext =
			static_cast<VkDebugUtilsMessengerCreateInfoEXT*>(&m_DebugMessengerInfo);
	}
	else
	{
	...
	ErrCheck(vkCreateInstance(&instanceInfo, nullptr, &m_VulkanInstance) != VK_SUCCESS,
		"Failed to create Vulkan instance!");

	// setup debug messenger
	if (Config::enableValidationLayers)
	{
		ErrCheck(inits::CreateDebugUtilsMessengerEXT(
					 m_VulkanInstance, &m_DebugMessengerInfo, nullptr, &m_DebugMessenger)
					 != VK_SUCCESS,
